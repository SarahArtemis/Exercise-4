...
print(start_text)
guessed_letters<- c()
letters_left<- paste("Letters left:", paste(setdiff(LETTERS, toupper(guessed_letters)), collapse = " ")) 
print(letters_left)
guess<- readline(prompt = cat("enter guess number", nguess, "here then press <enter>:")) 
guess
evaluate_guess <- function(guess=guess, solution=solution) { 
  
  splitguess<-strsplit(guess, split = "", fixed = F)
  splitguess<- toupper(splitguess)
  if (splitguess == solution) {
...
feedback<- paste( "Correct letters:", correct_letters, "Correct positions:", correct_positions, "Correct letters in wrong positions:", correct_wrong_positions)

  }
guessed_letters<- append(guessed_letters, unlist(splitguess))
evaluate_guess(guess = guess, solution = solution)
}
